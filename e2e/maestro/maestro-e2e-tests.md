...
| REG-164 | `164-ai-settings-and-relevance-sort.yaml` | `content-rich` | Android | AI is off by default: "Most Relevant" is absent from the sort sheet entirely. Enabling AI reveals the provider fields and shows the sort option disabled with the missing-key hint; turning it back off hides it again. No network: only the enable/key gating is asserted. |
...
- **Real AI summarisation and relevance scoring (REG-164)** — both need a live provider API key and a paid request per run, so no flow asks for a summary or a score. REG-164 covers the parts that are deterministic offline: the default-off state, the enable switch, and the sort option's visible/disabled gating. The request, response parsing, batching and score mapping are covered by unit tests instead (`OpenAiCompatibleArticleAiServiceTest`, `ArticleRelevanceRepositoryTest`).
